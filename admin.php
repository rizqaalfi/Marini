...
session_start();
include "konek.php";
$user  =  "SELECT * FROM user WHERE level='user'";
$keyword = "";
...
<body>
    ...
    <?php } else { ?>
        ...
        <div>
            <center>
                ...
                <form action="" method="post">
                    <div>
                        <input type="text" name="keyword" size="40" placeholder="Masukkan keyword" autocomplete="off" value="<?php echo $keyword ?>">
                        <button type="submit" class="w3-button w3-khaki w3-round" name="cari">Cari</button>
                    </div>
                </form>

                <div class="w3-table w3-striped w3-hoverable ">
                    <table style="width:50%" class="w3-card" id="resep">
        ...
        <?php
        $no = 1;
        $sql = mysqli_query($conn, $user) or die(mysqli_error($conn));
        while ($data = mysqli_fetch_array($sql)) { ?>
            <tr align="center">
                <td><?= $no++ ?>.</td>
                <td><?= $data['id_user'] ?></td>
                <td>
                    <?php echo "<img  src='img/profile/$data[foto_user]' width='80' height='80' alt='foto.$data[username]'>"; ?>

                </td>
                <td><?= $data['username'] ?></td>
                <td>
                    <a href="deluser.php?id=<?= $data['id_user'] ?>" onclick="return confirm('yakin akan menghapus data?')" class="btn fa fa-trash">
                    </a>
                <td>
            </tr>
